...
export class Star {
    ...
    reset = () => {
        ...
        const velocity: number = VELOCITY_BASE;
    ...
    update = (jumping) => {
        const x = Math.floor(Math.sin(this.STATE.characteristics.angle) * this.STATE.distance.travelled) + innerWidth / 2;
        const y = Math.floor(Math.cos(this.STATE.characteristics.angle) * this.STATE.distance.travelled) + innerHeight / 2;
...
export class HyperSpace {
    ...
    render = () => {
        const { context } = this;
        context.save();
        if (!this.STATE.jumping && !this.STATE.toBeCleared) context.clearRect(0, 0, window.innerWidth, window.innerHeight);
        ...
        if (!this.STATE.jumping && this.STATE.toBeCleared) {
            ...
            if (Math.random() > 0.5 && this.STATE.starPool.filter((s) => s.STATE.active === false).length > 0 && !this.STATE.jumping) {
